...
function makeIntentGateTargets(baseUrl: string): TestTarget[] {
  ...
  return [
    ...
    },
    {
      ...
      description: 'Do not treat creative video help as coding task intake just because it says create.',
      defaultRequest: requestJson({
        method: 'POST',
        url,
        body: { content: 'I want to create a short video about how to train large language model, can you help me with that?' },
        expectJsonOneOf: {
          'result.route': ['normal_chat', 'unknown_or_ambiguous'],
          'result.workflow': ['chat', 'clarify'],
          'result.requiresTaskIntake': [false],
        },
      }),
    },
...
function makeWorkflowDispatcherTargets(baseUrl: string): TestTarget[] {
  ...
  return [
    ...
    },
    {
      ...
      kind: 'workflow',
      name: 'workflow.creative_video_chat',
      label: 'creative_video_chat',
      description: 'Dispatch creative video help to chat/clarify instead of starting task intake.',
      defaultRequest: requestJson({
        method: 'POST',
        url,
        body: { content: 'I want to create a short video about how to train large language model, can you help me with that?' },
        expectJsonOneOf: {
          'result.classification.route': ['normal_chat', 'unknown_or_ambiguous'],
          'result.action': ['run_chat', 'ask_clarifying_question'],
          'result.workflow': ['chat', 'clarify'],
          'result.actionPreview.requiresConfirmation': [false],
        },
      }),
