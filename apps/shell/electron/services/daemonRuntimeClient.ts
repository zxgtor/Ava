...
export const daemonRuntimeClient = {
  ...
  unitTestContext: (states: unknown) => client().unitTestContext(states),
  appendUnitTestResult: (entry: unknown) => client().appendUnitTestResult(entry),
  readUnitTestResults: () => client().readUnitTestResults(),
  clearUnitTestResults: () => client().clearUnitTestResults(),
